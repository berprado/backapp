...
def imprimir_recibo(html_content, cuf, nit, numero_factura):
    logging.info("Iniciando la generación de la impresión con formato ESC/POS...")
    logging.debug(f"Contenido HTML (primeros 100 caracteres): {html_content[:100]}...")
    logging.debug(f"Longitud del contenido HTML: {len(html_content)}")
    logging.info(f"CUF: {cuf}, NIT: {nit}, Número de Factura: {numero_factura}")
    ...
    logging.info("Factura HTML guardada como final_factura_test.html")
...
def imprimir_recibo1(html_content, cuf, nit, numero_factura):
    ...
    with open("final_factura_test.html", "w", encoding="utf-8") as f:
        f.write(html_content)
    logging.info("HTML final guardado en final_factura_test.html")
...
def guardar_recibo_como_pdf(html_content, file_path):
    ...
    try:
        ...
        logging.info(f"Factura PDF guardada exitosamente en {file_path}")
    except Exception as e:
        logging.error(f"Error al guardar la factura como PDF: {e}")