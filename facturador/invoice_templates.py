...
def generate_html_invoice(factura_obj):
    """Genera el HTML de la factura en formato compacto a partir de FacturaProcesada."""
    if not isinstance(factura_obj, FacturaProcesada):
    ...
    return generate_compact_html_invoice(
...
def generate_compact_html_invoice(
    ...
    # Generar el código QR en base64 si tenemos el CUF
    qr_code_html = ""
    ...
    html_content += f"""
            ...
            <tr>
                <th colspan="2" id="leyenda" class="header">
                    <span style="font-weight:bold" id="leyenda_text">{leyenda}</span>
                </th>
            </tr>
                <tr>
