...
logging.debug(f'Variables de entorno cargadas: WSDL_URL_SYNC={wsdl_url}, API_KEY={api_key}, '
...
def sincronizar_lista_mensajes_servicios():
    mydb = None  # Inicializar mydb como None para asegurarse de que está definida

    logging.debug('Iniciando sincronización de lista de mensajes de servicios...')
    ...
    try:
        # Llamar al método sincronizarListaMensajesServicios
        response = client.service.sincronizarListaMensajesServicios(solicitud)
        ...
        cursor.execute("""
        CREATE TABLE IF NOT EXISTS sincronizarListaMensajesServicios (
        ...
        """)
        logging.debug('Tabla sincronizarListaMensajesServicios verificada/creada.')
        ...
        if response.listaCodigos:
            for codigo in response.listaCodigos:
                ...
                sql = """
                INSERT INTO sincronizarListaMensajesServicios (codigoClasificador, descripcion, fecha_creacion, fecha_sincronizacion, estado_sincronizacion)
...
if __name__ == "__main__":
    ...
    sincronizar_lista_mensajes_servicios()
