...
def sincronizar_lista_leyendas_factura():
    mydb = None  # Inicializar mydb como None para asegurarse de que está definida

    logging.debug('Iniciando sincronización de lista de leyendas de factura...')
    ...
    try:
        # Llamar al método sincronizarListaLeyendasFactura
        response = client.service.sincronizarListaLeyendasFactura(solicitud)
        ...
        cursor.execute("""
        CREATE TABLE IF NOT EXISTS sincronizarlistaleyendasfactura (
            id INT(11) NOT NULL AUTO_INCREMENT,
            codigoActividad VARCHAR(255) DEFAULT NULL,
            descripcionLeyenda TEXT DEFAULT NULL,
        ...
        """)
        logging.debug('Tabla sincronizarlistaleyendasfactura verificada/creada.')

        if response.listaLeyendas:
            for leyenda in response.listaLeyendas:
                logging.info(f'Procesando leyenda: {leyenda}')
                codigoActividad = leyenda.codigoActividad
                descripcionLeyenda = leyenda.descripcionLeyenda

                # Verificar si la leyenda ya existe
                cursor.execute(
                    "SELECT id FROM sincronizarlistaleyendasfactura WHERE codigoActividad = %s AND descripcionLeyenda = %s",
                    (codigoActividad, descripcionLeyenda),
                )
                leyenda_existente = cursor.fetchone()

                if leyenda_existente:
                    # Actualizar fecha de sincronización
                    cursor.execute(
                        """
                        UPDATE sincronizarlistaleyendasfactura
                        SET fecha_sincronizacion = NOW(), estado_sincronizacion = 'Exitoso'
                        WHERE id = %s
                        """,
                        (leyenda_existente[0],)  # Usar el ID de la leyenda existente
                    )
                else:
                    # Insertar nueva leyenda
                    sql = """
                    INSERT INTO sincronizarlistaleyendasfactura (codigoActividad, descripcionLeyenda, fecha_creacion, fecha_sincronizacion, estado_sincronizacion)
                    VALUES (%s, %s, NOW(), NOW(), 'Exitoso')
                    """
                    val = (codigoActividad, descripcionLeyenda)
            ...
            logging.info("No se encontraron nuevas leyendas, se actualizó la fecha de sincronización de las existentes.")
...
if __name__ == "__main__":
    ...
    sincronizar_lista_leyendas_factura()
